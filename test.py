...
class ViewAndMutationMetaFromDynamo(torch._dynamo.test_case.TestCase):
    ...
    def test_cdist_batch(self):
        ...
        p = 2.5
        p = float('inf')
        x = torch.randn(2, 3, 6, 3, 2)
        y = torch.randn(2, 3, 6, 4, 2)
        actual = torch.compile(fn)(x, y, p)
        expected = torch.compile(fn2)(x, y, p)
        no_compile = fn(x, y, p)

        print(f"Actual checksum {actual.sum()} vs expected checksum {expected.sum()}")
        self.assertEqual(expected, actual)
        self.assertEqual(no_compile, actual)
